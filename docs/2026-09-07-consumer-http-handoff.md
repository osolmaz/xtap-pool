...
Each read has a 60-second wall deadline, including source/context I/O and mutex wait. A request that reaches an exact full-selection coverage calculation or reads pages from a changed source gets a five-minute absolute deadline measured from the original request start. Bootstrap, history, reconciliation, and all other stages keep the 60-second deadline. Clients must allow at least 315 seconds for changed-source responses and responses that can calculate full-selection coverage. SQLite runs in at most two persistent read-only child processes. Deadline cancellation kills a running child, including native SQLite. Requests do not copy or rebuild the database. The database must contain every exact pinned source descriptor and the current verified source boundary.
...
On the same local frozen database, the fixed initial coverage took 14.597 seconds. The earlier unbounded diagnostic took 4.459 and 4.330 seconds for completion queries and 24.792 seconds for observation coverage. These are single local measurements, not cloud latency estimates. The decision boundary remains the existing 30-second request limit, with unchanged row/privacy checks. The complete corrected production-selection canary passed: 576 pages, 42,633 conversations, 84,570 distinct posts, and 111,763,946 response bytes. Request time totalled 702.085 seconds; median was 1.114 seconds, p95 was 1.736 seconds, and the maximum was 15.496 seconds. Twenty unchanged polls and a reader restart emitted zero changes; unchanged p95 was 2.598 seconds. Selected content was complete through `2026-09-07T11:02:05.771Z`. This is one local frozen-source run, not cloud or live-website performance. No source write, paid call, or Job was required.

Review at `ac8f29102706926fc302fba1cca14aae2771f000` found no issues. [CI run 34148170156](https://github.com/osolmaz/xtap-pool/actions/runs/34148170156) passed. The follow-up is [pull request 48](https://github.com/osolmaz/xtap-pool/pull/48).
