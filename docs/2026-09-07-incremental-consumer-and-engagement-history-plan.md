...
Pi Reviewer found one P1 in the first pass: the extension required an `ok` flag that the Space's successful ingest response does not return. The extension now validates the existing `added`, `duplicates`, and `rejected` fields, and its normal queue test uses that exact response shape. The second review found no issues. CI then exposed a separate clean-build error: forked consumer processes were not compiled before tests. Root and Space test/coverage commands now build them first. The combined checks and all 518 Space tests passed after that correction. The third review found no issues, and CI passed on `d573fd4`. The configured mutation targets also passed at 82.51%; these targets do not establish mutation coverage for the new consumer modules. Extension version 0.26.0 identifies the new repeat-observation behavior; it has not been installed in the live browser.

A subsequent code audit found an acceptance gap: every idle change poll still requested a coverage calculation, and `selectedObservationThrough` read historical body JSON across the selected cohort. An empty HTTP response therefore did not prove zero unrelated body reads. The next correction must reuse proven unchanged coverage and calculate metric coverage from affected posts while preserving exact source, selection, privacy, and completion rules. This work is in progress; do not treat the passing review and CI as proof that the performance acceptance matrix is complete.

The real local source preparation saved 10,368 of 38,219 segments before a network read failed. A bounded restart read four additional segments, not the completed prefix, and produced a valid paused checkpoint at 10,372 segments. Its database held 400,822 logical observations and occupied 3,222,917,120 bytes. Preparation resumed from that point. This is recovery evidence, not a completed bootstrap or a live deployment. Computing the classifier contract with the deployed GLM-5.2 model and built-in taxonomy gives the same `2a2814ad4162457c0b8fe6de9cc07e0813f9fffadd3bde21ca46c731cc58de69` hash as the incumbent index.

The semantic schema checks are incomplete. Schemator's real Codex strategy timed out first at 120 seconds and again at 600 seconds per field. The longer attempts returned three of 39 SQL-field reviews and seven of ten observation-field reviews. Aggregation correctly reported incomplete coverage; no proposal was applied and no converged schema approval is claimed. In particular, the extracted `unknown` counter type is an extraction limitation: the production validators already require nullable, nonnegative safe integers. HTTP and Our Models schemas remain outside that partial result.
...
`index:bootstrap` now separates preparation from publication. Preparation freezes an exact source snapshot beside the working SQLite database and commits bounded, chronologically ordered chunks. A restart checks that the database is an exact prefix of that target and reads only the remaining segment bodies. An existing database is never silently replaced. An incomplete-bootstrap marker travels inside SQLite and blocks serving or publication, including after the database is copied without its companion snapshot file. Publication also rejects a candidate that omits a previously published source segment. The default chunk is 128 segments; `INDEX_BOOTSTRAP_MAX_SEGMENTS` bounds a diagnostic run. SIGINT and SIGTERM stop after a saved chunk. These are local SQLite checkpoints, not remote Job checkpoints.

`INDEX_BOOTSTRAP_MODE=publish` is a separate operator action. It requires complete preparation and, when a live index exists, its exact `INDEX_BOOTSTRAP_EXPECTED_DATABASE` value. It applies the remaining source tail, verifies the full database, uploads and verifies it, and then replaces the pointer. Other publishers must be stopped for this cutover. Preparation does not grant publication approval or change a schedule. Setup uses the same two stages and retains partial work after failure.

Full SQLite integrity checks now remain at restore, explicit verification, and publication. Normal tail transactions still verify raw identities and segment projection counts, but do not rescan every SQLite page, including after a harmless source check.

A real local process-interruption canary used the audited 42 tweet segments. The process was killed with SIGKILL after 14 committed segments. Restart read only the remaining 28 segments (197,323 compressed bytes), preserved all 1,196 logical observations, and matched a fresh replay's observation, content, and physical-source digests. The fresh replay read 263,173 compressed bytes and produced a 7,405,568-byte database. It took 0.595 seconds on this 20-core ARM64 host with about 128 MiB peak process RSS; restart took 0.462 seconds for its remaining work. No remote state or live pointer changed, and no model or paid Job ran. This small tweet-only fixture proves local recovery, not production-sized performance, classifier-result coverage, or remote Job recovery. The full source includes mixed/result/registry records and 38,219 network objects, so these times must not be extrapolated as a production completion estimate.

A read-only source pilot then prepared the first 128 real segments of the audited full snapshot. These contain 104,831,277 compressed bytes, 293,280 tweet records and logical observations, 196,276 raw enrichment rows, 372,876 attempt events, 22,122 registry events, and 76 receipts. These are whole-source counts, not the Our Models cohort or proof of its eligibility results. Two bounded invocations took 12.64 and 288.73 seconds. The saved database is 2,500,280,320 bytes; peak RSS in the larger invocation was 1,356,296 KiB. There were no model calls, paid Jobs, remote writes, or live pointer changes.

The full pinned-source preparation completed on September 7. It contains all 38,219 segments, 689,421 logical observations, 326,102 raw enrichment rows, 636,129 attempt events, 48,081 registry events, and 195 receipts. The verified SQLite candidate is 4,811,792,384 bytes. These are whole-source counts, not the Our Models selection. The candidate remains local at `space/.data/consumer-bootstrap-2026-09-07/index/bootstrap.sqlite`; no production pointer changed.

A network read failed after 10,368 saved segments. A four-segment recovery probe then completed, and the final continuation read only the remaining 27,847 segments (92,462,211 compressed bytes). That continuation, including final verification, took 6,213.060 seconds and reached 573,592 KiB peak process RSS. This is not the total time across the initial pilot, failed attempt, probe and continuation. Preparation used 128-segment checkpoints and eight concurrent reads on the same 20-core ARM64 host. Actual new HF Job and model-call cost is $0.00 because neither ran; ordinary host power and existing storage charges are excluded. No runtime or service was installed.

Full-source reader tests exposed and corrected query-order and result-history bounds that smaller fixtures did not reveal. Both code changes are merged in [#46](https://github.com/osolmaz/xtap-pool/pull/46) and [#47](https://github.com/osolmaz/xtap-pool/pull/47), with passing review and CI. The [HTTP handoff](2026-09-07-consumer-http-handoff.md#full-source-initial-coverage-fix) records the complete local read, restart, no-op, history, body-read and final query-parity evidence, including the slower repeated timings. These tests do not establish cloud latency or website freshness. Deployment still requires source-tail catch-up, current predecessor checks, observation grants and the coordinated Our Models cutover. The canonical enrichment and publication schedules remained active and non-concurrent at 15:07 UTC.
