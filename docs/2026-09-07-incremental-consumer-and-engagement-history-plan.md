...
The remaining 38,091 pinned segments contain 127,564,561 compressed bytes. Their largest 128-segment chunk is 2,911,288 compressed bytes. Continued local preparation reuses the saved prefix, uses 128-segment checkpoints and eight concurrent reads, caps the JavaScript heap at 4 GiB, and stops after six hours. The planning range is two to six hours and a final 4–10 GB database, not a completion claim; network latency and the different remaining record mix limit extrapolation. Expected additional HF Job and inference cost is $0.00 because neither is used. This excludes ordinary host power and existing storage charges. No inference runtime or service is installed. A completed local candidate still needs final index verification and the coordinated publication gate.
