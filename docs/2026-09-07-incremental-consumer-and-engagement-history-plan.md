...
Pi Reviewer found one P1 in the first pass: the extension required an `ok` flag that the Space's successful ingest response does not return. The extension now validates the existing `added`, `duplicates`, and `rejected` fields, and its normal queue test uses that exact response shape. The second review found no issues. CI then exposed a separate clean-build error: forked consumer processes were not compiled before tests. Root and Space test/coverage commands now build them first. The combined checks and all 518 Space tests passed after that correction; review and CI must be repeated for the corrected head. Extension version 0.26.0 identifies the new repeat-observation behavior; it has not been installed in the live browser.
