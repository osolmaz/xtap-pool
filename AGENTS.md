...
- Never inject or execute code in a website's main world, replace or wrap website APIs, or modify page-owned code. Capture website signals through passive browser-level listeners from extension-owned contexts. The vendored `lib/graphql-capture.js` already uses passive Chrome Debugger Network listeners. Reuse that path; do not restore the removed main-world fetch/XHR interception.
